...
router.get('/organization/membership', validateToken, async (req, res) => {
    ...
    try{
        const organization = await Organization.findOne({
            attributes:['id','membership_period','strict'],
            where: {userId: id}
        });
...
router.post('/membership/status', validateToken, async (req, res) => {
    const {id} = req.decoded;
    ...
    try{
        ...
        res.json({success: 'Successfully updated membership status!'});
    }catch(err){
        res.json(err);
    }
});

// Update organization strict status
router.post('/organization/strict', validateToken, async (req, res) => {
    const {id} = req.decoded;
    ...
    try{
        ...
        res.json({success: 'Successfully updated organization strict status!'});
    }catch(err){
        res.json(err);
    }
});
